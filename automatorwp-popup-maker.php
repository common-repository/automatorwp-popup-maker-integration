...
final class AutomatorWP_Popup_Maker_Integration {
    ...
    private function constants() {
        // Plugin version
        define( 'AUTOMATORWP_POPUP_MAKER_VER', '1.0.4' );
    ...
    public function admin_notices() {
        ...
        if ( ! $this->meets_requirements() && ! defined( 'AUTOMATORWP_ADMIN_NOTICES' ) ) : ?>
            ...
            <div id="message" class="notice notice-error is-dismissible">
                <p>
                    <?php printf(
                        __( 'AutomatorWP - Popup Maker requires %s and %s in order to work. Please install and activate them.', 'automatorwp-popup-maker-integration' ),
                        '<a href="https://wordpress.org/plugins/automatorwp/" target="_blank">AutomatorWP</a>',
                        '<a href="https://wordpress.org/plugins/popup-maker/" target="_blank">Popup Maker</a>'
                    ); ?>
                </p>
            </div>
        ...
        <?php elseif ( $this->pro_installed() && ! defined( 'AUTOMATORWP_ADMIN_NOTICES' ) ) : ?>
            ...
            <div id="message" class="notice notice-error is-dismissible">
                <p>
                    <?php echo __( 'You can uninstall AutomatorWP - Popup Maker Integration because you already have the pro version installed and includes all the features of the free version.', 'automatorwp-popup-maker-integration' ); ?>
                </p>
            </div>
    ...
    public function load_textdomain() {
        ...
        $lang_dir = AUTOMATORWP_POPUP_MAKER_DIR . '/languages/';
        $lang_dir = apply_filters( 'automatorwp_popup_maker_languages_directory', $lang_dir );

        // Traditional WordPress plugin locale filter
        $locale = apply_filters( 'plugin_locale', get_locale(), 'automatorwp-popup-maker-integration' );
        $mofile = sprintf( '%1$s-%2$s.mo', 'automatorwp-popup-maker-integration', $locale );

        // Setup paths to current locale file
        $mofile_local   = $lang_dir . $mofile;
        $mofile_global  = WP_LANG_DIR . '/automatorwp-popup-maker-integration/' . $mofile;

        if( file_exists( $mofile_global ) ) {
            // Look in global /wp-content/languages/automatorwp-popup-maker-integration/ folder
            load_textdomain( 'automatorwp-popup-maker-integration', $mofile_global );
        } elseif( file_exists( $mofile_local ) ) {
            // Look in local /wp-content/plugins/automatorwp-popup-maker-integration/languages/ folder
            load_textdomain( 'automatorwp-popup-maker-integration', $mofile_local );
        } else {
            // Load the default language files
            load_plugin_textdomain( 'automatorwp-popup-maker-integration', false, $lang_dir );
        }
